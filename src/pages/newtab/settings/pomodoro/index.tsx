...
const Pomodoro = () => {
  const { presets, preset, changePreset } = usePomodoroSetting(
    useShallow(state => ({
  ...
  return (
    <div className="space-y-2">
      ...
      <RadioGroup className="w-full gap-3" defaultValue={preset} onValueChange={changePreset}>
        {presets.map(({ name, value, focus, sort, long, description }) => {
          ...
          return (
            <div
              ...
              {value === 'custom' ? (
                <PresetCustom focus={focus} long={long} sort={sort} />
              ) : (
                <StackedBar focus={focus} sort={sort} long={long} />
              )}
            </div>
          )
        })}
